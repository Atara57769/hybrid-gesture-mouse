...
def draw_hud(frame, current_class, is_recording, counts):
    ...
    cv2.rectangle(overlay, (15, 15), (320, 260), (30, 30, 30), -1)
    ...
    cv2.putText(frame, "ACTIVE TRACKING ZONE", (az_x_min + 5, az_y_min - 8), 
    ...
    # HUD Title
    cv2.putText(frame, "GESTURE DATA COLLECTOR", (25, 40), 
    ...
    cv2.putText(frame, status_text, (35, 78), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0) if not is_recording else (255, 255, 255), 2, cv2.LINE_AA)
    
    # Current Class label
    cv2.putText(frame, f"Active State:", (25, 110), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.55, (200, 200, 200), 1, cv2.LINE_AA)
    cv2.putText(frame, f"{CLASSES[current_class].upper()} ({current_class})", (135, 110), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 180, 50), 2, cv2.LINE_AA)
    
    # Class sample counts table
    cv2.putText(frame, "Collected samples:", (25, 135), 
    ...
    for code, name in CLASSES.items():
        ...
        cv2.putText(frame, f"[{code}] {name.capitalize()}:", (35, y_offset), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, thickness, cv2.LINE_AA)
        cv2.putText(frame, f"{counts.get(code, 0)}", (220, y_offset), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (100, 255, 100) if counts.get(code, 0) > 0 else (100, 100, 100), 2, cv2.LINE_AA)
        y_offset += 20
        
    # Instructions Overlay at bottom right
    inst_overlay = frame.copy()
    cv2.rectangle(inst_overlay, (width - 290, height - 120), (width - 15, height - 15), (20, 20, 20), -1)
    ...
    cv2.putText(frame, "[Space] Start/Pause recording", (width - 280, height - 67), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1, cv2.LINE_AA)
    cv2.putText(frame, "[C] Clear selected class dataset", (width - 280, height - 52), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1, cv2.LINE_AA)
    cv2.putText(frame, "[Q] Save and Quit", (width - 280, height - 37), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (100, 150, 255), 1, cv2.LINE_AA)
...
def main():
    ...
    while cap.isOpened():
        ...
        if is_recording and not has_hand:
            cv2.putText(frame, "WARNING: NO HAND DETECTED", (400, 50),
        ...
        if key == ord('q') or key == ord('Q'):
            ...
            counts[current_class] = temp_counts[current_class] + sum(1 for row in session_data if row[0] == current_class)
